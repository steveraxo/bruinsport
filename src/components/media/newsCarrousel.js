...
class Header extends Component {
  ...
  nextPage(event) {
    ...
    const elements = [
      ...document.querySelectorAll(`.news__list_element.${type}`),
    ]
    ...
    return (
      <div className={`news__list ${this.props.state}`} id={this.props.elId}>
        {newsArray.map((element, index) => (
          <div
            className={`${this.props.elId} news__list_element ${
              element.title !== null ? "" : "hidden"
            }`}
            key={`${this.props.elId}-${index}`}
          >
                ...
                <>
                  {element.acf !== null && element.acf.pdf_press_release ? (
                    ...
                    >
                      <img
                        src={downloadIcon}
                        alt="Download the PDF attachment"
                      />
                    </a>
                  ) : (
                    ""
                      ...
                      {element.acf.media_file ? (
                        ...
                        <>
                          {element.acf.external_news_link !== null ? (
                            <a
                              href={element.acf.external_news_link.source_url}
                              target="_BLANK"
                              rel="noopener noreferrer"
                            >
                              <img src={linkIcon} alt="Link to Press Release" />
        ...
        <div className="news__list__pagination">
          <div
            ...
            tabIndex={0}
            data-finalpage={`${Math.ceil(
              newsArray.length / this.state.paginationSize
            )}`}
          >
            {" "}
            back
